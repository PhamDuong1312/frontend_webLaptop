...
import { bankingAPi, deleteProToCart, getCartUser, orderApi, updateCart } from "../../../services/Api"
import { VND, checkEmail, checkPhone, getImageProduct } from "../../../ultils"
...
const Cart = () => {
    ...
    const getData = () => {
        getCartUser(user._id, {}).then(({ data }) => {
            setCart(data)
            const qty = data.reduce((sum, item) => sum + item.quantity, 0)
            setQuantity(qty)
            const sumPrice = data.reduce((sum, item) =>
                sum + (item.product.price - (item.product.price * item.product.giamGia) / 100) * item.quantity, 0)
            setTotal(sumPrice)
        })
    }
                                                    ...
                                                    {cart.map((item) => {
                                                        ...
                                                        return (
                                                            <tr className="cart_item">
                                                                <td className="cart-product-name"> {item.product.name}<strong className="product-quantity"> × {item.quantity}</strong></td>

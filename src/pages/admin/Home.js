...
import { VND, renderId } from "../../ultils";
...
const Home = () => {
    ...
    const getDataPiechart=async ()=>{
        const data1=await getOrders({ params: { filter: "pending" } })
        const data2=await getOrders({ params: { filter: "shipping" } })
        const data3=await getOrders({ params: { filter: "done" } })
        return [data1.data.total, data2.data.total, data3.data.total]
    }
    React.useEffect( () => {
        getAllUsers({})
            .then(({ data }) => setUser(data.total))
        getAllProducts({})
            .then(({ data }) => setProduct(data.total))
        getAllComments({})
            .then(({ data }) => setComment(data.total))
        getOrders({ params: { filter: "pending" } })
            .then(({ data }) => setOrder(data.total))
        getOrdersByDate({ params: dateInput })
            .then(({ data }) => {
                setOrders(data)
                const tongTien = data.reduce((total, item) => total + item.totalPrice, 0)
                setTotal(tongTien)
            })
                                        ...
                                        if (index < 5) {
                                            return (
                                                <div style={{ width: "80%", border: "1px solid #ccc", borderRadius: 10, padding: "10px 10px 0 10px", marginBottom: 30 }}>
                                                    ...
                                                    <p style={{ textAlign: "right", borderBottom: "1px solid #ccc", padding: "10px 0" }}>Tổng tiền : {VND.format(item.totalPrice)}</p>
